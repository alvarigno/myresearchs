    ...
    public class ParsearController : ApiController
    {
        ...
        public HttpResponseMessage Post()
        {
            ...
            if (listaEmails.Count > 0)
            {
                ...
                foreach (var itemEmail in listaEmails)
                {
                    ...
                    if (bProcesar)
                    {
                        ...
                        */
                        ...
                        string strCadenaParseada = lDatosEmails.Where(email => email.ReglaAplicada).Aggregate("", (current, email) => current + (email.UidEmail + "~" + email.Contenido + "~" + email.TipoCampo + "~" + email.Nombre + "¬"));
                ...
                strMensaje += "'Procesados':'" + Procesados + " emails','Parseados':'"+Parseados+"','Con observación':'"+ ConObservacion+"','Sin parsear':'"+ NoParseados+"'";
            }
            else
            {
