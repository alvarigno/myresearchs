    ...
    public partial class tbk_normal : System.Web.UI.Page
    {
        ...
        protected void Page_Load()
        {
            ...
            if (action == "init")
            {
                ...
                ChCookie.Values.Add("urlformorigen", Request.Form[keys[13]]);
            ...
            string htmlfin = "</br><a href='" + Request.Cookies["ChileautosurlFormOrigenTBK"]["urlformorigen"] + "'>&laquo; volver al formulario de transacciones.</a>"+
            "</div>"+
            "</div>"+
            "</div>"+
            "</div>";
            ...
            switch (action)
            {
                ...
                default:
                    ...
                    try
                    {
                        ...
                        /** validación que no oc log con el mismo nombre **/
                        string curFile = Path.GetDirectoryName(System.AppDomain.CurrentDomain.BaseDirectory) + "\\logsrequest\\" + buyOrder + "_log.txt";
...
//                        }
                    ...
                    catch (Exception ex)
                    {
                        ...
                        HttpContext.Current.Response.Write("<tr><td><button class='btn btn-warning btn-large btn-block' onclick=\"location.href='"+ Request.Cookies["ChileautosurlFormOrigenTBK"]["urlformorigen"] + "';\">Regresar al formulario de pago.</button></td></tr>");
                        HttpContext.Current.Response.Write("</table>");
                        HttpContext.Current.Response.Write("</div>");
                        HttpContext.Current.Response.Write(htmlfin);
                    ...
                    try
                    {
                        ...
                        if (Request.Cookies["ChileautosSettingTBK"] != null)
                        {
                            if (Request.Cookies["ChileautosSettingTBK"]["authorizationCode"] != null)
                            {
                                ...
                                HttpCookie ChurlformCookie = new HttpCookie("ChileautosurlFormOrigenTBK");
                                ChurlformCookie.Expires = DateTime.Now.AddDays(-1d);
                                Response.Cookies.Add(ChurlformCookie);
